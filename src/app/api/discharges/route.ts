...
function transformPatientData(
  ...
  if (laceScore.risk_level === "high")
    riskFactors.push("high_readmission_risk");
  if (laceScore.risk_level === "moderate")
    riskFactors.push("moderate_readmission_risk");
  if (medications.length >= 5) riskFactors.push("polypharmacy");
  if (encounter.encounter_type === "emergency")
    riskFactors.push("emergency_admission");

  // Check for specific comorbidities
  const comorbidities =
    laceScore.charlson_comorbidity_index?.comorbidities || [];
  comorbidities.forEach((comorbidity) => {
    if (comorbidity.cci_mapping) {
      riskFactors.push(
        comorbidity.cci_mapping.toLowerCase().replace(/\s+/g, "_")
      );
    }
  });
  ...
  if (laceScore.total_score) {
    labResults[
      "LACE Score"
    ] = `${laceScore.total_score} (${laceScore.risk_level})`;
  }
  ...
  return {
    ...
    secondary_diagnoses:
      patientData.discharge_diagnoses?.map((d) => d.description) || [],
