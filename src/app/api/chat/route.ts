...
async function getPatientData(): Promise<DischargeSummary[]> {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/discharges`
  );
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (patientIds && patientIds.length > 0) {
      ...
      relevantPatients = allPatients.filter(patient => patientIds.includes(patient.id));
    } else {
    ...
    console.log(
      `Filtered ${allPatients.length} total patients down to ${relevantPatients.length} relevant patients for query: "${message}"`
    );
      ...
      try {
        ...
        if (hasGeminiKey) {
          ...
          try {
            ...
            const geminiResponse = await geminiClient.generateInsights(
              message,
              relevantPatients,
              chatHistory
            );
    ...
    if (error instanceof Error) {
      if (error.message.includes('API key')) {
        ...
        errorMessage =
          'No LLM API keys configured. Please set OPENAI_API_KEY or GEMINI_API_KEY in your environment variables.';
      }
    }
